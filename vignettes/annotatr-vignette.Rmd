...
## Gene Annotations from Any TxDb

The genic annotations above come from the `TxDb.*` packages that `annotatr` knows about. To build the same annotation types from other gene models, e.g. GENCODE or RefSeq, or for a genome without built-in annotations, use `build_txdb_annotations()` with any `TxDb` or `EnsDb` object. A GTF or GFF3 file can be made into a `TxDb` with `txdbmaker::makeTxDbFromGFF()`.
