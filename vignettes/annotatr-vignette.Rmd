...
The development version of `annotatr` is available from [Bioconductor devel](https://bioconductor.org/packages/devel/bioc/html/annotatr.html), which needs the devel version of R and Bioconductor:

```{r, eval=FALSE}
BiocManager::install("annotatr", version = "devel")
```

The source is on [GitHub](https://github.com/rcavalcante/annotatr). Changes in each version are listed in the package's NEWS, `news(package = "annotatr")`.
...
availability = annotatr::builtin_annotations_table()
availability[-1] = lapply(availability[-1], function(x) ifelse(x, '✓', ''))
knitr::kable(availability, align = c('l', rep('c', ncol(availability) - 1)))
```
...
The ENCODE registry of candidate cis-regulatory elements (cCREs), from SCREEN ([ENCODE Project Consortium et al., 2020](https://doi.org/10.1038/s41586-020-2493-4)), is available for hg38 and mm10, version 4. cCREs are 150–350 bp regions of accessible chromatin, classified by their histone marks, CTCF, and transcription factor binding:
...
The `[genome]_ccres` shortcut builds all 8 classes. The `id` column is ENCODE's ID for each cCRE, its accession (e.g. `EH38E2776516`), which can be looked up in [SCREEN](https://screen.wenglab.org). There are about 2.3 million cCREs for hg38 and 0.9 million for mm10, most of them distal enhancer-like, so annotations with all classes are large, and plots of all 8 classes can be busy.
...
The long non-coding RNA (lncRNA) transcripts are from [GENCODE](https://www.gencodegenes.org) for hg19 (release 19), hg38 (release 31), and mm10 (release M6). The `tx_id` column is the Ensembl transcript ID, `ensembl_id` is the Ensembl gene ID, `symbol` is the gene symbol, and `gene_id` and `entrez_id` are the Entrez Gene ID matched by symbol. The transcript [biotype](https://www.gencodegenes.org/pages/biotypes.html), from GENCODE's `transcript_type` field, is at the start of the `id` column.
