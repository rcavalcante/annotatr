...
Also included in genic annotations are intronexon and exonintron boundaries. These annotations are 200bp up/down stream of any boundary between an exon and intron. Expanded regions are then merged as necessary.
...
FANTOM5 permissive enhancers were determined from bi-directional CAGE transcription as in [Andersson et al. (2014)](http://www.nature.com/nature/journal/v507/n7493/full/nature12787.html), and are downloaded and processed for **hg19** and **mm10** from the [FANTOM5](http://fantom.gsc.riken.jp/5/datafiles/phase2.0/extra/Enhancers/) resource.

## Custom Annotations

Users may load their own annotations from BED files using the `read_annotations()` function, which uses the `rtracklayer::import()` function. The output is a `GRanges` with `mcols()` for `id`, `gene_id`, `symbol`, and `type`. If a user wants to include `gene_id` and `symbol` in their custom annotations they can be included as extra columns on a BED6 input file.
...
annots = c('hg19_cpgs', 'hg19_basicgenes', 'hg19_genes_intergenic',
    ...
    'hg19_custom_ezh2', 'hg19_custom_cjun')
...
```{r, fig.align='center', fig.cap='Number of DM regions per pair of annotations.', fig.height=7, fig.width=7, fig.show = 'hold', warning = FALSE, message = FALSE}
